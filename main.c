...
//    Usart_Tx('a');
    ...
    // If we 'were' in running mode (GO_NOWWAIT) state then make sure we jump straight back to that state
    // and ignore the 'up/plus' button as we have to tie high this button to make the boot loader jump
    // immediately in to the user program
    if (isButtonUpOn()) {
      state = ST_GONOWWAIT_FUNC;
      nextstate = ST_GONOWWAIT_FUNC;
      statetext = NULL;
      pStateFunc = GoNowWaitFunc;

      gCOUNTER = ((uint32_t)getCounterMinCheckpoint())*60;
      // Make sure there is always at least an hour otherwise
      // it could constantly give out fud if the power kept cycling
      if (gCOUNTER == 0) {
        gCOUNTER = 3600;
      }
      gInGo = TRUE;
...
//    DF_CS_inactive;             // disable DataFlash
    ...
    OUTS_init();  // initialize the output ports
